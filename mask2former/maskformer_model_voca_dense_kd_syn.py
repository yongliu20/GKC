...
class OpenSegMaskFormerDenseTextKDSyn(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        attn_shape = self.backbone.output_shape()['attn'][0] if 'attn' in self.backbone.output_shape() else 1024
        vision_in_dim = {
            "resAtten": attn_shape,
        ...
        if self.kd_super:
            ...
            if self.kd_stu_source == 'prior_relu':
                ...
                for i in range(dec_layers - 1):
                    aux_kd_proj = nn.Sequential(
                        nn.ReLU(),
                        nn.Linear(in_dim, tec_dim)
                    )
                    setattr(self, "aux_kd_porj_{}".format(i), aux_kd_proj)
                ...
                for i in range(dec_layers - 1):
                    aux_kd_proj = nn.Linear(in_dim, tec_dim)
                    setattr(self, "aux_kd_porj_{}".format(i), aux_kd_proj)
            elif self.kd_stu_source == 'posterior':
                self.kd_proj = nn.Linear(embed_dim, tec_dim)
                for i in range(dec_layers - 1):
                    aux_kd_proj = nn.Linear(embed_dim, tec_dim)
                    setattr(self, "aux_kd_porj_{}".format(i), aux_kd_proj)
            else:
                self.kd_proj = None
        ...
        if self.training:
            ...
            if self.syn_super:
                if self.syn_process_each:
                    ...
                    total_cls_num = syn_class_ebmd.shape[1] + 1
                ...
                loss_syn = self.loss_syn_label(cls_pred, targets, indices, valid_mask, syn_class_slice)
                losses.update({"loss_syn": loss_syn})

                for i, aux_query in enumerate(aux_vision_ebmd):
                    aux_query_norm = aux_query / (aux_query.norm(dim=-1, keepdim=True) + 1e-7)
                    aux_cls_pred = torch.bmm(aux_query_norm * logit_scale, syn_class_ebmd)

                    aux_loss_syn = self.loss_syn_label(aux_cls_pred, targets, aux_indices[i], valid_mask, syn_class_slice)
                    losses.update({"loss_syn_{}".format(i) : aux_loss_syn})
            ...
            if self.kd_super:
                ...
                kd_v_idx_b, _ = self.criterion._get_src_permutation_idx(indices)
                ...
                if self.additional_text_kd:
                    plain_kd_loss, text_kd_loss = self.loss_text_kd(kd_vision_embed, masked_clip_features, kd_text_ebmd, indices, accm_num_list)
                    losses.update({"loss_kd" : plain_kd_loss, "loss_text_kd" : text_kd_loss})
                else:
                    kd_loss = self.loss_text_kd(kd_vision_embed, masked_clip_features, kd_text_ebmd, indices, accm_num_list)
                    losses.update({"loss_kd" : kd_loss})
                
                for i, a_indices in enumerate(aux_indices):
                    if hasattr(self, "aux_kd_porj_{}".format(i)):
                        a_kd_proj = getattr(self, "aux_kd_porj_{}".format(i))
                    if 'prior' in self.kd_stu_source:
                        aux_kd_v_feat = a_kd_proj(seg_feature['query_feature'][i])
                    elif 'posterior' in self.kd_stu_source:
                        aux_kd_v_feat = a_kd_proj(aux_vision_ebmd[i])
                    else:
                        aux_kd_v_feat = aux_vision_ebmd[i]

                    # TODO align to main indices, same tgt(gt) order
                    t_a_indices = self.indice2main(a_indices, indices)

                    kd_v_idx_b, kd_v_idx_i = self.criterion._get_src_permutation_idx(t_a_indices)
                    aux_kd_v_embed = aux_kd_v_feat[kd_v_idx_b, kd_v_idx_i]    # (n1+n2+...+nn), dim
                    aux_kd_v_embed = F.normalize(aux_kd_v_embed, p=2, dim=1)

                    if cur_len > self.max_kd_len:
                        aux_kd_v_embed = aux_kd_v_embed[kd_idx]

                    if self.additional_text_kd:
                        plain_kd_loss, text_kd_loss = self.loss_text_kd(aux_kd_v_embed, masked_clip_features, kd_text_ebmd, t_a_indices, accm_num_list)
                        losses.update({"loss_kd_{}".format(i) : plain_kd_loss, "loss_text_kd_{}".format(i) : text_kd_loss})
                    else:
                        kd_loss = self.loss_text_kd(aux_kd_v_embed, masked_clip_features, kd_text_ebmd, t_a_indices, accm_num_list)
                        losses.update({"loss_kd_{}".format(i) : kd_loss})

            # grouding supervision
            if self.grounding_super:
                loss_g = self.loss_ground(vision_ebmd, indices, text_ebmd, text_mask, logit_scale)
                losses.update({"loss_grounding" : loss_g})

                for i, a_v_embd in enumerate(aux_vision_ebmd):
                    loss_g = self.loss_ground(a_v_embd, aux_indices[i], text_ebmd, text_mask, logit_scale)
                    losses.update({"loss_grounding_{}".format(i) : loss_g})
    ...
    def loss_text_kd(self, kd_vision_embed, masked_clip_features, kd_text_ebmd, indices, accm_num_list):
        ...
        if self.additional_text_kd:
            ...
            loss = plain_kd_loss, text_kd_loss
        else:
    ...
    def loss_ground(self, vision_ebmd, indices, text_ebmd, text_mask, logit_scale, max_g_v_num=25):
        # max_g_v_num = 25
        ...
        if self.grounding_gather:
            ...
            gather_vision_g_ebmd = [torch.ones_like(vision_g_ebmd)
                for _ in range(word_size)]
            dist.all_gather(gather_vision_g_ebmd, vision_g_ebmd, async_op=False)
            gather_vision_g_ebmd[cur_rank] = vision_g_ebmd
            gather_vision_g_ebmd = torch.cat(gather_vision_g_ebmd, dim=0)
            ...
            grounding_loss = self.grounding_head(gather_text_ebmd, gather_vision_g_ebmd, gather_text_mask, gather_vision_ebmd_mask, temperature=logit_scale)
        else:
            grounding_loss = self.grounding_head(text_ebmd, vision_g_ebmd, text_mask, vision_ebmd_mask, temperature=logit_scale)
        
        return grounding_loss["loss_grounding"]
