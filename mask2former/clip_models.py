...
@BACKBONE_REGISTRY.register()
class CLIPResNetWithAttention(Backbone):
    """
    A ResNet class that is similar to torchvision's but contains the following changes:
    - There are now 3 "stem" convolutions as opposed to 1, with an average pool instead of a max pool.
    - Performs anti-aliasing strided convolutions, where an avgpool is prepended to convolutions with stride > 1
    ...
    def __init__(self, cfg, input_shape):
        ...
        output_dim       = cfg.MODEL.RESNETS.OUTPUT_DIM
        input_resolution = cfg.MODEL.RESNETS.INPUT_RESOLUTION
        width            = cfg.MODEL.RESNETS.WIDTH
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            u, w = self.load_state_dict(state_dict, False)
            # print(key_list, 'are loaded from CLIP params')
            print(u, w, 'are misaligned params in CLIPResNet')
        else:
            print(pretrained, "is not a path to CLIP params")
...
class CLIPVisionTransformer(Backbone):
    ...
    # def __init__(self, input_resolution=640, patch_size=16, width=768, layers=12, heads=12, output_dim=512, drop_path_rate=0.1, out_indices=[3, 5, 7, 11], pretrained=None, get_embeddings=True, **kwargs):
    def __init__(self, cfg, input_shape):
        input_resolution = cfg.MODEL.VIT.INPUT_RESOLUTION
        patch_size = cfg.MODEL.VIT.PATCH_SIZE
        width = cfg.MODEL.VIT.WIDTH
        layers = cfg.MODEL.VIT.LAYERS
        heads = cfg.MODEL.VIT.HEADS
        output_dim = cfg.MODEL.VIT.OUTPUT_DIM
        drop_path_rate = cfg.MODEL.VIT.DROP_PATH_RATE
        out_indices = cfg.MODEL.VIT.OUT_INDICES
        pretrained = cfg.MODEL.VIT.PRETRAINED
        get_embeddings = cfg.MODEL.VISION_FEATURE == 'resAtten'
...
class CLIPTextEncoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        transformer_width  = cfg.MODEL.TEXT_ENCODER.TRANSFORMER_WIDTH
        transformer_heads  = cfg.MODEL.TEXT_ENCODER.TRANSFORMER_HEADS
        transformer_layers = cfg.MODEL.TEXT_ENCODER.TRANSFORMER_LAYERS
        embed_dim          = cfg.MODEL.TEXT_ENCODER.EMBED_DIM
        out_dim            = cfg.MODEL.TEXT_ENCODER.OUT_DIM
    ...
    def forward(self, text):
        bs, n_seq, n_ctx = text.shape
        text = text.view(bs * n_seq, n_ctx)
        ...
        # x = self.out_proj(x)
        x = x.view(bs, n_seq, self.embed_dim)
        return x
